#Import libraries & modules (uses pygame external library)
import pygame
...
pygame.display.set_caption("CompBuilder App")
icon = pygame.image.load('CB_Logo_Wordless.png')
pygame.display.set_icon(icon)
background = pygame.image.load('Background.png')
helpPage = pygame.image.load('helpPage.png')

#Icons
Play = pygame.image.load('Play.png')
compMode = pygame.image.load('compositionMode.png')
Clear = pygame.image.load('Trash.png')
helpIcon = pygame.image.load('helpIcon.png')
Stop = pygame.image.load('Stop.png')
pianoMode = pygame.image.load('pianoMode.png')
backButton = pygame.image.load('Back.png')
tempoIcon = pygame.image.load('tempo.png')

#Functions that place icons
def homeIcons():
    screen.blit(helpIcon,(700,10))
def defaultIcons():
    screen.blit(compMode,(353,364))
def compIcons():
    screen.blit(Clear,(650,365))
    screen.blit(Stop,(75,368))
    screen.blit(pianoMode,(348,366))
    screen.blit(restFont.render('(REST: Spacebar, TEMPO: Up/down arrows)',True,(0,0,0)),(160,320))
    screen.blit(tempoIcon,(50,50))
    screen.blit(Play,(30,367))
def homeDisplay():
...
restFont = pygame.font.SysFont('arial',20,italic=True,bold=True)
whiteTextName = ['Z','X','C','Q','W','E','R','B','N','M','Y','U','I','O','P']
whiteTextX = [18,68,118,168,218,268,318,368,418,468,518,568,618,668,710]
whiteTextY = 466
blackTextName = ['S','D','2','3','4','H','J','7','8','9']
blackTextX = [29,85,180,236,292,379,436,530,586,642]
blackTextY = 415

#Places texts
def keyTexts():
    for i in range(15):
        screen.blit(font.render(whiteTextName[i],True,(190,160,140)),(whiteTextX[i],whiteTextY))
    for i in range(10):
        screen.blit(font.render(blackTextName[i],True,(190,160,140)),(blackTextX[i] + 9,blackTextY))
...
def audioRun():
    ...
    for note in workspace:
        ...
        if pygame.mixer.get_busy() == False:
            ...
            #waits for audio to run before playing next note
...
def compose(key):
    ...
    if len(workspace) < 72:
        ...
        #Moves to next available x position
...
while running:
    for event in pygame.event.get():
        ...
        if home:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if(cx > 700 and cx < 718  and cy > 10 and cy < 38):
                    home = False
                if(cx > 325 and cx < 375 and cy > 364 and cy < 394):
            ...
            if composeMode:
                if event.type == pygame.KEYDOWN:
                    compose(event.key)
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if(cx > 30 and cx < 50  and cy > 365 and cy < 390):
                        audioRun()
                    if(cx > 650 and cx < 675  and cy > 365 and cy < 395):
                        clear()
                compIcons()
                display()
        ...
        #Help page
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if(cx > 10 and cx < 70  and cy > 10 and cy < 50):
                    home = True 
            helpDisplay()
